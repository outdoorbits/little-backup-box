...
<script>
	async function openMatrixTokenFetcher() {
		...
		const MATRIX_TOKEN_PRESENTER	= document.getElementById('MATRIX_TOKEN_PRESENTER');
		...
		if (!dlg) {
			...
			dlg.querySelector('#mx-gettoken-fetch').onclick = async (e) => {
				...
				try {
					...
					// keep dialog open for visible message
				} catch (e) {
					console.error(e);
			...
			dlg.querySelector('#mx-gettoken-choose').onclick	= (e) => {
				...
				if (tokenEl) {
					...
					MATRIX_TOKEN_PRESENTER.value	= tkInput.value;
				}
...
<input type="hidden" id="conf_SOCIAL_MATRIX_TOKEN" name="conf_SOCIAL_MATRIX_TOKEN" value="<?php echo htmlspecialchars($config['conf_SOCIAL_MATRIX_TOKEN'] ?? '', ENT_QUOTES); ?>">

<label for="MATRIX_TOKEN_PRESENTER"><?php echo L::config_social_matrix_token_label; ?></label><br />
<input type="text" id="MATRIX_TOKEN_PRESENTER" name="MATRIX_TOKEN_PRESENTER" size="40" value="<?php
	if (!empty($config['conf_SOCIAL_MATRIX_TOKEN'])) {
		echo htmlspecialchars($config['conf_SOCIAL_MATRIX_TOKEN'], ENT_QUOTES);
	}
	?>" disabled>
