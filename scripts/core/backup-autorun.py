...
class backup_autorun(object):
	...
	def __default_backup(self):
		...
		conf_BACKUP_RENAME_FILES				= self.__setup.get_val('conf_BACKUP_RENAME_FILES')
		conf_BACKUP_DEFAULT2_MOVE_FILES			= self.__setup.get_val('conf_BACKUP_DEFAULT2_MOVE_FILES')
		conf_BACKUP_GENERATE_THUMBNAILS			= self.__setup.get_val('conf_BACKUP_GENERATE_THUMBNAILS')
		conf_BACKUP_UPDATE_EXIF					= self.__setup.get_val('conf_BACKUP_UPDATE_EXIF')
		...
		if conf_BACKUP_DEFAULT_SOURCE != 'none' and conf_BACKUP_DEFAULT_TARGET != 'none':
			PrimaryBackupConfig	= [
				...
				f'--rename-files', f'{conf_BACKUP_RENAME_FILES}',
				f'--force-sync-database', 'False',
				f'--generate-thumbnails', f'{conf_BACKUP_GENERATE_THUMBNAILS}',
				f'--update-exif', f'{conf_BACKUP_UPDATE_EXIF}',
