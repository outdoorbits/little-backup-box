...
class socialmedia(object):
	...
	def get_upload_times(self):
		return([] if not self.SERVICE_Obj else self.SERVICE_Obj.upload_times)
	...
	def get_order_by(self):
		return('DESC' if self.SERVICE_Obj and self.SERVICE_Obj.newPostsOnTop else 'ASC')
	...
	def publish(self, Comment='', FilePath=None, Create_Date=''):
		...
		if msgtype.main and self.SERVICE_Obj:
			while self.SERVICE_Obj.rate_limit_count and len(self.SERVICE_Obj.upload_times) > self.SERVICE_Obj.rate_limit_count:
				self.SERVICE_Obj.upload_times.pop(0)
