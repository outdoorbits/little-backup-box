		...
		try:
			async def send_text(text, formatted=None):
				...
				if isinstance(resp, self.RoomSendResponse):
					...
					self.ok				= False
					self.add_error_message(f"room_send error: {resp}")
					return(False)
			...
			if msgtype.main == 'text':
				...
				async def upload_file(path: Path):
					...
					if isinstance(upload_resp, self.UploadResponse):
						...
						self.ok	= False
						self.add_error_message( f"upload error: {upload_resp}")
						return None, None
				...
				for index, CommentPart in enumerate(CommentParts):
					if index == 0 and media_uri:
						...
						if not isinstance(resp, self.RoomSendResponse):
							self.ok	= False
							self.add_error_message( f"room_send media error: {resp}")
							break
					else:
						if CommentPart:
							ok	= await send_text(CommentPart)
				...
				self.ok	= False
				self.add_error_message(f'unsupported msgtype {msgtype.main}{"" if msgtype.sub is None else f" ({msgtype.sub})"}')
