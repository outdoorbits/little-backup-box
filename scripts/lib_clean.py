...
class clean(object):
	...
	def display_content(self):
		try:
			subprocess.run(f"/usr/bin/bash -c 'shopt -s nullglob; /usr/bin/rm -f {self.__const_DISPLAY_CONTENT_PATH}/*'", shell=True)
		except:
			pass
	...
	def display_image(self):
		...
		if not self.__conf_DIPLAY_IMAGES_KEEP:
			try:
				subprocess.run(f"/usr/bin/bash -c 'shopt -s nullglob; /usr/bin/rm -f -R {self.__const_DISPLAY_IMAGE_KEEP_PATH}'", shell=True)
			except:
				pass

	def tasks(self):
		try:
			subprocess.run(f"/usr/bin/bash -c 'shopt -s nullglob; /usr/bin/rm -f {self.__const_TASKS_PATH}/*'", shell=True)
		except:
			pass
