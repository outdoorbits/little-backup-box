...
<div id="app">
    <div x-data='{ sidebarOpen: false }' class='flex h-screen bg-gray-200 dark:bg-gray-900'>
        <div :class="sidebarOpen ? 'block' : 'hidden'" @click='sidebarOpen = false'
             class='fixed z-20 inset-0 bg-black opacity-50 transition-opacity lg:hidden'></div>

        <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class='fixed z-30 inset-y-0 left-0 w-64 transition duration-300 transform bg-gray-900 dark:bg-gray-800 overflow-y-auto lg:translate-x-0 lg:static lg:inset-0 rounded-xl lg:ml-5 my-5 p-2'>
        ...
        <div class='flex-1 flex flex-col overflow-hidden'>
            <header class='flex justify-between items-center py-6 px-6 border border-gray-300 rounded-2xl overflow-hidden m-5'>
                ...
                <div class='flex items-end'>
                    <a class='text-grey-700 py-2 px-4 border-r border-gray-300 dark:text-gray-200' href=''><?= L::help; ?></a>
                    <form method='POST' class='mb-0'>
                        <button name='reboot' class='text-grey-700 py-2 px-4 border-r border-gray-300 dark:text-gray-200'><?= L::main_reboot_button; ?></button>
                        <button name="shutdown" class="text-grey-700 py-2 px-4 dark:text-gray-200"><?= L::main_shutdown_button; ?></button>
                    </form>
                </div>
            </header>