...
# Set the PWR LED to heartbeat to indicate that the backup is in progress
sudo sh -c "echo heartbeat > /sys/class/leds/PWR/trigger"
		...
		if [[ " usb internal ios " =~ " ${SOURCE_MODE} " ]]; then
			...
			if [ "${TARGET_MODE}" = "rsyncserver" ]; then
				...
				if [ $conf_LOG_SYNC = true ]; then
					sudo sshpass -p "${conf_RSYNC_PASSWORD}" rsync ${RSYNC_OPTIONS} --mkpath --no-perms --min-size=1 --exclude "*.id" --exclude "*tims/" --exclude "${const_IMAGE_DATABASE_FILENAME}" --log-file="${const_LOGFILE_SYNC}" "${SOURCE_PATH}/" "${RSYNC_CONNECTION}/${BACKUP_PATH}/" | syncprogress "rsync" "${SOURCE_FOLDER_INFO}"
					SYNC_RETURN_CODE="${PIPESTATUS[0]}"
					SYNC_LOG="${SYNC_LOG}\n$(<"${const_LOGFILE_SYNC}")"
					log_pick_file "${const_LOGFILE_SYNC}"
				else
					sudo sshpass -p "${conf_RSYNC_PASSWORD}" rsync ${RSYNC_OPTIONS} --mkpath --no-perms --min-size=1 --exclude "*.id" --exclude "*tims/" "${SOURCE_PATH}/" --exclude "${const_IMAGE_DATABASE_FILENAME}" "${RSYNC_CONNECTION}/${BACKUP_PATH}/" | syncprogress "rsync" "${SOURCE_FOLDER_INFO}"
					SYNC_RETURN_CODE="${PIPESTATUS[0]}"
				fi
